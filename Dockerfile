...
RUN mkdir -p /tmp/src
WORKDIR /tmp/src
RUN git clone https://github.com/babyplus/coordinates_playback.git

FROM registry.cn-hangzhou.aliyuncs.com/babyplus/get:a240725cebee.node.20_16_0-bullseye as compiler
COPY --from=downloader /tmp/src/coordinates_playback /tmp/src/coordinates_playback
WORKDIR /tmp/src/coordinates_playback
RUN yarn install
RUN yarn build

FROM registry.cn-hangzhou.aliyuncs.com/babyplus/get:a221123f5c48.nginx.1_23
COPY --from=compiler /tmp/src/coordinates_playback/dist /usr/share/nginx/html
